...
var $COMPILER_MACROS, $LOAD, $OUT, $PACKAGE, $RUNTIME, List, Map, Method, RT, Set, Symbol, Tag, Var, apply, baseNS, baseSymbol, concat, cons, contains, drop, dropWhile, eighth, fifth, filter, first, foldl, forEach, fourth, fromArray, get, isEmpty, map, newline, ninth, nth, partition, pr, print, println, prn, put, raise, represent, representArray, rest, second, seventh, sixth, size, take, takeWhile, third, toArray, toType, toTypeName, toUID, _cat;
RT = {};
baseNS = "gandalf";
...
filter.extend(null, function(_, _) {
  return null;
}, String, function(f, s) {
  var buf, c, i, _i, _len;
  buf = [];
  ...
  return toType(xs).fromArray(filter(f, toArray(xs)));
});
first = Method({
  name: 'first'
});
...
nth.extend(Method.DEFAULT, function(n, xs) {
  return toArray(xs)[n - 1];
}, String, function(n, s) {
  return s[n - 1];
}, Array, function(n, xs) {
  return xs[n - 1];
...
SpecialForm.define({
  ...
  "let*": function(e, x) {
    ...
    fin = List.create(baseSymbol("let*"), res, expandBody(e, x.tail));
    return fin;
  },
  "letrec*": function(e, x) {
    var body;
    body = toArray(x.tail);
    return expandLetRec(e, x.head, x.tail);
  },
  "js*": function(e, x) {
...
expandLetRec = function(e, defs, body) {
  var bindings, exprs, i, names, pair, res, x, _i, _j, _len, _len2, _len3;
  defs = toArray(defs);
  body = toArray(body);
  e = e.extend();
  names = [];
  ...
  res = List.create(baseSymbol("letrec*"), bindings, expandBody(e, body));
  return res;
};
...
bootstrap(baseNS, {
  ...
  "array-like?": function(x) {
    return x instanceof Object && __indexOf.call(x, 'length') >= 0;
  },
  "true?": function(x) {
    return x === true;
  },
